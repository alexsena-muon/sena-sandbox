import argparse
from multiprocessing import Process, Queue, Event, Lock
import socket
import fcntl
import time
import crcmod
import subprocess
import serial
import struct
import random
import select
import queue
import os
import json
import re
from smbus2 import SMBus, i2c_msg

from mfc_gpio_util import gpio_get, gpio_set

HAVE_GPS = True

def set_scheduler(nice):
    # param = os.sched_param(os.sched_get_priority_max(os.SCHED_FIFO))
    # os.sched_setscheduler(
    #    0, os.SCHED_RR, os.sched_param(param.sched_priority + nice + 1)
    # )
    os.nice(nice)

def send_data(name, output, data):
    print(data)
    data_string = " ".join("{}:{}".format(key, data[key]) for key in sorted(data))
    output.put(f"{name} {data_string}")


######################################################################
#                   UPTIME
######################################################################

MEMINFO_KEYS = set(("MemTotal", "MemFree", "MemAvailable", "Buffers", "Cached"))


def uptime(name, output, running, lock, nice):
    """Once every 15 seconds read uptime and meminfo and report to console"""
    set_scheduler(nice)

    data = {}
    while running.is_set():
        (uptime,) = list(open("/proc/uptime"))
        (data["uptime"], data["idle"]) = uptime.strip().split()

        meminfo = list(open("/proc/meminfo"))

        for l in meminfo:
            s = l.strip().split()
            if len(s) != 3:
                continue
            key, value, unit = s
            assert unit == "kB", l
            assert key[-1] == ":", l
            key = key[:-1]

            # Discard some of meminfo data, don't need it all!
            if key not in MEMINFO_KEYS:
                continue

            data[key.upper()] = value

        send_data(name, output, data)
        time.sleep(15)

######################################################################
#                   BOARDMON
######################################################################

# INA3221 has 3 shunt/bus voltage pairs.
INA3221_PAIRS = [(0x01, 0x02), (0x03, 0x04), (0x05, 0x06)]


def ina3221_voltage(bus_reg):
    """Convert bus voltage to voltage

    >>> ina3221_voltage(struct.unpack('>h', b'\\x7F\\xF8')[0])
    32.76

    """
    return (bus_reg >> 3) * 0.008


def ina3221_current(shunt_reg, shunt_resistance):
    """Convert shunt voltage to current

    >>> ina3221_current(struct.unpack('>h', b'\\xC1\\x80')[0], 1)
    -0.08
    >>> ina3221_current(struct.unpack('>h', b'\\x7F\\xF8')[0], 1)
    0.1638

    """
    shunt_voltage = (shunt_reg >> 3) * 40e-6

    # Convert shunt voltage to current
    return shunt_voltage / shunt_resistance


def ina3221(bus, address, *channel_names_and_shunt_resistances, init=False):
    if init:
        return

    data = {}

    for (name, shunt_resistance), (shunt_voltage_reg, bus_voltage_reg) in zip(
        channel_names_and_shunt_resistances, INA3221_PAIRS
    ):
        # Prepare read of shunt and bus voltage for pair.
        shunt_write = i2c_msg.write(address, [shunt_voltage_reg])
        shunt_read = i2c_msg.read(address, 2)

        bus_write = i2c_msg.write(address, [bus_voltage_reg])
        bus_read = i2c_msg.read(address, 2)

        # Do reads together
        bus.i2c_rdwr(shunt_write, shunt_read)
        bus.i2c_rdwr(bus_write, bus_read)

        (shunt_reg,) = struct.unpack(">h", shunt_read.buf[: shunt_read.len])
        (bus_reg,) = struct.unpack(">h", bus_read.buf[: bus_read.len])

        data[f"{name}_V"] = "{:.3f}".format(ina3221_voltage(bus_reg))
        data[f"{name}_A"] = "{:.3f}".format(
            ina3221_current(shunt_reg, shunt_resistance)
        )

    return data


DEG_C_TO_DEG_K = 273.15


def at30ts75a_temperature(temp_reg):
    """Convert AT30TS75A temperature from register to deg K.

    >>> at30ts75a_temperature(struct.unpack('>h', b'\\x7D\\x00')[0])
    398.15
    >>> at30ts75a_temperature(struct.unpack('>h', b'\\x00\\x10')[0])
    273.2125
    >>> at30ts75a_temperature(struct.unpack('>h', b'\\x00\\x00')[0])
    273.15
    >>> at30ts75a_temperature(struct.unpack('>h', b'\\xFF\\xF0')[0])
    273.0875
    >>> at30ts75a_temperature(struct.unpack('>h', b'\\xCD\\x80')[0])
    222.65
    >>> at30ts75a_temperature(struct.unpack('>h', b'\\xC9\\x00')[0])
    218.15


    """
    temp_milli_deg_c = (temp_reg >> 4) * 62.5
    return (temp_milli_deg_c + DEG_C_TO_DEG_K * 1000) / 1000


def at30ts75a(bus, address, sensor_name, init=False):
    if init:
        # Configure temperature sensor to 12-bits (200ms conversion time).
        conf_write = i2c_msg.write(address, [0x01, 0x60, 0x00])
        bus.i2c_rdwr(conf_write)
        return

    temp_write = i2c_msg.write(address, [0x0])
    temp_read = i2c_msg.read(address, 2)

    bus.i2c_rdwr(temp_write, temp_read)

    (temp_reg,) = struct.unpack(">h", temp_read.buf[: temp_read.len])

    data = {}
    data[sensor_name] = at30ts75a_temperature(temp_reg)
    return data


def ltc2946_voltage(bus_reg):
    """Convert ADC V_in register to voltage (V)

    >>> ltc2946_voltage(0x0)
    0.0
    >>> ltc2946_voltage(0x10)
    0.025
    >>> ltc2946_voltage(0xfff0)
    102.375

    """
    return (bus_reg >> 4) * 0.025


def ltc2946_current(shunt_reg, shunt_resistance):
    """Convert ADC delta SENSE register to current (A)

    >>> ltc2946_current(0x0, 1)
    0.0
    >>> ltc2946_current(0x10, 1)
    2.5e-05
    >>> ltc2946_current(0xfff0, 1)
    0.10237500000000001

    """
    return (shunt_reg >> 4) * (25e-6 / shunt_resistance)


def ltc2946_power(power_reg, shunt_resistance):
    """Convert POWER register to power (W)

    >>> ltc2946_power(0x0, 1)
    0.0
    >>> ltc2946_power(0x1, 1)
    6.25e-07

    """

    # POWER reg is bus reg (1 LSB = 0.025) * shunt reg (1 LSB = 25e-6)
    return power_reg * (6.25e-7 / shunt_resistance)


def ltc2946_adin(adin_reg):
    """Convert ADIN register to voltage (V)

    >>> ltc2946_adin(0x0)
    0.0
    >>> ltc2946_adin(0x10)
    0.0005
    >>> ltc2946_adin(0xFFF0)
    2.0475

    """
    return (adin_reg >> 4) * 0.5e-3


def ltc2946(bus, address, name, shunt_resistance, adin_name, adin_gain, init=False):
    if init:
        if adin_name is None:
            # Configure V, A sensing
            conf_write = i2c_msg.write(address, [0x0, 0x18])
        else:
            # Configure ADIN, V, A sensing
            conf_write = i2c_msg.write(address, [0x0, 0x1B])

        bus.i2c_rdwr(conf_write)
        return

    data = {}

    shunt_write = i2c_msg.write(address, [0x14])
    shunt_read = i2c_msg.read(address, 2)

    bus_write = i2c_msg.write(address, [0x1E])
    bus_read = i2c_msg.read(address, 2)

    power_write = i2c_msg.write(address, [0x05])
    power_read = i2c_msg.read(address, 3)

    bus.i2c_rdwr(shunt_write, shunt_read)
    bus.i2c_rdwr(bus_write, bus_read)
    bus.i2c_rdwr(power_write, power_read)

    (shunt_reg,) = struct.unpack(">H", shunt_read.buf[: shunt_read.len])
    (bus_reg,) = struct.unpack(">H", bus_read.buf[: bus_read.len])
    (power_reg,) = struct.unpack(">I", power_read.buf[: power_read.len] + b"\x00")
    power_reg >>= 8

    data[f"{name}_V"] = "{:.3f}".format(ltc2946_voltage(bus_reg))
    data[f"{name}_A"] = "{:.3f}".format(ltc2946_current(shunt_reg, shunt_resistance))
    data[f"{name}_P"] = "{:.3f}".format(ltc2946_power(power_reg, shunt_resistance))

    if adin_name is not None:
        adin_write = i2c_msg.write(address, [0x28])
        adin_read = i2c_msg.read(address, 2)

        bus.i2c_rdwr(adin_write, adin_read)

        (adin_reg,) = struct.unpack(">H", adin_read.buf[: adin_read.len])

        data[adin_name] = "{:.3f}".format(ltc2946_adin(adin_reg) * adin_gain)

    return data


SENSOR_CYCLE_TIME = 1  # seconds
HEATER_FORCE_ON_TIME = 3  # seconds
HEATER_CYCLE_TIME = 240  # seconds

SENSORS = [
    # MPU LTC3545
    (
        0x40,
        ina3221,
        (
            ("VDD_1V0_AUX", 0.05),
            ("VDD_2V5_AUX", 0.05),
            ("VDD_3V3_GPS", 0.05),
        ),
    ),
    # MPU LTC3853
    (
        0x41,
        ina3221,
        (
            ("VDD_3V3_M2M", 0.01),
            ("VDD_3V3_M2M2", 0.01),
        ),
    ),
    # Power Switches
    #(
    #    0x42,
    #    ina3221,
    #    (
    #         ("VDD_5V0_HSIO_USB", 0.05),
    #         ("BLANK", 0.05),
    #         ("VDD_5V0_HSIO_MIPI", 0.05),
    #    ),
    #        ),
    (0x48, at30ts75a, ("TEMP0",)),
    (0x49, at30ts75a, ("TEMP1",)),
    (0x4A, at30ts75a, ("TEMP2",)),
    (0x4B, at30ts75a, ("TEMP3",)),
    # ADIN on HSIO has a 1/3 voltage divider
    (0x6A, ltc2946, ("VDD_28V0_TTCFE0", 0.02, "VDD_28V0_TTCFE0_V", 16.8)),
    (0x6B, ltc2946, ("VDD_28V0_TTCFE1", 0.02, "VDD_28V0_TTCFE1_V", 16.8)),
    (0x6C, ltc2946, ("VDD_28V0_HSIO", 0.02, "VDD_5V0_HSIO_V", 3)),
    (0x6D, ltc2946, ("VDD_28V0_MFC", 0.02, "VDD_5V0_V", 3)),
]

def board_heater_enable(enable):
    gpio_set("HEAT_EN", enable)


def board_id():
    board_id = 0
    # TODO(jnutzmann): get this from the CM4.
    # board_id |= gpio_get("BOARD_ID0") << 0
    # board_id |= gpio_get("BOARD_ID1") << 1
    # board_id |= gpio_get("BOARD_ID2") << 2
    # board_id |= gpio_get("BOARD_ID3") << 3

    return board_id


def board_mon(name, output, running, lock, nice):
    set_scheduler(nice)

    bus = SMBus(16)

    board_heater_enable(False)
    heater_timer = time.monotonic()

    def board_heater(data):
        nonlocal heater_timer

        now = time.monotonic()
        time_since_last_force = now - heater_timer
        # For the first HEATER_FORCE_ON_TIME of a cycle, force the heater on.
        heater_force_state = time_since_last_force < HEATER_FORCE_ON_TIME

        # After the heater cycle time has elapsed, reset the timer.
        if time_since_last_force > HEATER_CYCLE_TIME:
            heater_timer += HEATER_CYCLE_TIME

        min_temp = min(float(data["TEMP{}".format(i)]) for i in range(4))
        min_temp -= DEG_C_TO_DEG_K
        heater_enable = (min_temp < 0) or heater_force_state
        board_heater_enable(heater_enable)
        data["HEATER_EN"] = int(heater_enable)

    for sensor, sensor_function, args in SENSORS:
        sensor_function(bus, sensor, *args, init=True)

    # Reset timer just before entering loop
    heater_timer = time.monotonic()
    prev = time.monotonic()

    try:
        while running.is_set():
            data = {}

            for sensor, sensor_function, args in SENSORS:
                data.update(sensor_function(bus, sensor, *args))

            board_heater(data)
            data["BOARD_ID"] = board_id()

            now = time.monotonic()
            sleep_time = max((prev + SENSOR_CYCLE_TIME) - now, 0.0)

            data["idle"] = "{:.3}".format(sleep_time)
            if sleep_time > 0:
                time.sleep(sleep_time)
                prev += SENSOR_CYCLE_TIME
            else:
                prev = now

            data_string = " ".join(
                "{}:{}".format(key, data[key]) for key in sorted(data)
            )
            output.put(f"{name} {data_string}")
    finally:
        board_heater_enable(False)

######################################################################
#                   PWR_SW
######################################################################

POWER_ENABLES = (
    ("HSIO_5V0", "HSIO_5V0_EN"),
    ("TTCFE0", "TTCFE0_DISABLE_L"),
    ("TTCFE1", "TTCFE1_DISABLE_L"),
)
SWITCH_TOGGLE_INTERVAL = 5 * 60  # seconds


def power_toggle(name, output, running, lock, nice):

    for switch_name, pin_name in POWER_ENABLES:
        gpio_set(pin_name, False)

    switch_state = [False for _ in POWER_ENABLES]
    data = {}
    for switch_name, _ in POWER_ENABLES:
        data[switch_name] = 0

    toggle_index = 0

    try:
        while running.is_set():
            time.sleep(SWITCH_TOGGLE_INTERVAL)

            switch_state[toggle_index] = not switch_state[toggle_index]
            toggle_index = (toggle_index + 1) % len(POWER_ENABLES)

            with lock:
                for (switch_name, pin_name), enabled in zip(
                    POWER_ENABLES, switch_state
                ):
                    gpio_set(pin_name, enabled)
                    data[switch_name] = int(enabled)

            send_data(name, output, data)
    finally:
        with lock:
            for switch_name, pin_name in POWER_ENABLES:
                gpio_set(pin_name, False)

######################################################################
#                   GPS
######################################################################

def set_gps_reset(lock, reset):
    with lock:
        gpio_set("GPS_RESET_L", not reset)


def set_gps_power(lock, power):
    with lock:
        gpio_set("GPS_PWR_EN", power)

def set_osc_power(lock, power):
    with lock:
        gpio_set("HSIO_5V0_EN", power)
        gpio_set("HSIO_MIPI_EN", power)


GPS_COM_PORT = 3
GPS_BOOT_TIMEOUT = 30  # seconds
GPS_REPORT_INTERVAL = 0.5  # seconds
GPS_BAUDRATE = 230400
GPS_LISTEN_TIME = 24 * 60 * 60  # seconds

# How often to send a command to check Tx path
GPS_COMMAND_INTERVAL = 10  # seconds

GPS_CRC = crcmod.mkCrcFun(poly=0x104C11DB7, initCrc=0, rev=True, xorOut=0)

HWMONITOR_TYPES = {
    0x01: "TEMP0",
    0x02: "ANT_CURR",
    0x06: "VDD_3V3",
    0x07: "ANT_VOLT",
    0x08: "VDD_1V2",
    0x0F: "REG_VOLT",
    0x11: "VDD_1V8",
    0x15: "VDD_5V0",
    0x16: "TEMP1",
    0x17: "VDD_CORE",
}

def gps_crc(data: bytes):
    """Computes GPS CRC over bytes

    # From "1.10 32-Bit CRC" in OEM7 Commands and Logs reference.
    >>> test_sequence = '''
    ... 0xAA, 0x44, 0x12, 0x1C, 0x2A, 0x00, 0x02, 0x20, 0x48, 0x00,
    ... 0x00, 0x00, 0x90, 0xB4, 0x93, 0x05, 0xB0, 0xAB, 0xB9, 0x12,
    ... 0x00, 0x00, 0x00, 0x00, 0x45, 0x61, 0xBC, 0x0A, 0x00, 0x00,
    ... 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x1B, 0x04, 0x50, 0xB3,
    ... 0xF2, 0x8E, 0x49, 0x40, 0x16, 0xFA, 0x6B, 0xBE, 0x7C, 0x82,
    ... 0x5C, 0xC0, 0x00, 0x60, 0x76, 0x9F, 0x44, 0x9F, 0x90, 0x40,
    ... 0xA6, 0x2A, 0x82, 0xC1, 0x3D, 0x00, 0x00, 0x00, 0x12, 0x5A,
    ... 0xCB, 0x3F, 0xCD, 0x9E, 0x98, 0x3F, 0xDB, 0x66, 0x40, 0x40,
    ... 0x00, 0x30, 0x30, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ... 0x00, 0x00, 0x0B, 0x0B, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03'''
    >>> bits = test_sequence.replace(' ', '\\n').replace(',', '\\n').split('\\n')
    >>> byte_sequence = bytes(int(b.strip(), 0) for b in bits if b)
    >>> hex(gps_crc(byte_sequence))
    '0x484cdc42'

    """
    return GPS_CRC(data)


def add_ascii_crc(s: str):
    """Adds CRC to GPS ASCII command

    >>> add_ascii_crc('''
    ... BESTPOSA,COM1,0,78.0,FINESTEERING,1427,325298.000,00000000,6145,2748;
    ... SOL_COMPUTED,SINGLE,51.11678928753,-114.03886216575,1064.3470,-16.2708,
    ... WGS84,2.3434,1.3043,4.7300,"",0.000,0.000,7,7,0,0,0,06,0,03'''
    ...    .replace('\\n',''))
    b'#BESTPOSA,COM1,0,78.0,FINESTEERING,1427,325298.000,00000000,6145,2748;SOL_COMPUTED,SINGLE,51.11678928753,-114.03886216575,1064.3470,-16.2708,WGS84,2.3434,1.3043,4.7300,"",0.000,0.000,7,7,0,0,0,06,0,03*9c9a92bb'

    """
    b = s.encode("ascii")

    return b"#" + b + "*{:08x}".format(gps_crc(b)).encode("ascii")


def remove_ascii_crc(b: bytes) -> str:
    """Deframe GPS ASCII encoded message

    >>> msg = ('''
    ... BESTPOSA,COM1,0,78.0,FINESTEERING,1427,325298.000,00000000,6145,2748;
    ... SOL_COMPUTED,SINGLE,51.11678928753,-114.03886216575,1064.3470,-16.2708,
    ... WGS84,2.3434,1.3043,4.7300,"",0.000,0.000,7,7,0,0,0,06,0,03'''
    ...    .replace('\\n',''))
    >>> good_msg = add_ascii_crc(msg)
    >>> remove_ascii_crc(good_msg) == msg
    True

    """

    if not b.startswith(b"#"):
        raise ValueError("Missing prefix?")

    if b[-9:-8] != b"*":
        raise ValueError("Missing suffix?")

    expected_crc = int(b[-8:].decode("ascii"), 16)

    if gps_crc(b[1:-9]) != expected_crc:
        raise ValueError("CRC error!")

    return b[1:-9].decode("ascii")


def gps(name, output, running, lock, nice):
    # Start with GPS unpowered, but out of reset.
    set_gps_reset(lock, reset=False)
    set_gps_power(lock, power=False)
    set_osc_power(lock, power=False)

    data = {
        "BOOT_TIMEOUTS": 0,
        "LOGS_RECEIVED": 0,
        "RX_ERRS": 0,
        "BYTES_TX": 0,
        "BYTES_RX": 0,
        "POWER_CYCLES": 0,
        "RESETS": 0,
    }

    last_report = time.monotonic()

    def check_report():
        nonlocal last_report
        now = time.monotonic()
        if now > last_report + GPS_REPORT_INTERVAL:
            send_data(name, output, data)
            last_report = now

    def ser_read_into(ser, buf):
        b = ser.read(256)
        data["BYTES_RX"] += len(b)
        buf.extend(b)

    def ser_write(ser, buf):
        written = ser.write(buf)
        data["BYTES_TX"] += written
        return written

    def wait_for_gps_ready(ser):
        buf = bytearray()
        expire = time.monotonic() + GPS_BOOT_TIMEOUT

        while running.is_set():
            check_report()

            timeout = 1.0
            rlist, _, _ = select.select([ser], [], [], timeout)

            check_report()

            if rlist:
                ser_read_into(ser, buf)

            if buf.endswith(f"[COM{GPS_COM_PORT}]".encode()):
                # GPS is alive
                return True

            if time.monotonic() > expire:
                data["BOOT_TIMEOUTS"] += 1
                return False

    log_buffer = bytearray()

    def yield_logs(ser, timeout):
        nonlocal log_buffer
        expire = time.monotonic() + timeout

        while running.is_set():
            if time.monotonic() > expire:
                raise TimeoutError()

            select_timeout = 1.0
            rlist, _, _ = select.select([ser], [], [], select_timeout)
            if rlist:
                ser_read_into(ser, log_buffer)

            check_report()

            while True:
                start = log_buffer.find(b"#")
                if start == -1:
                    break

                end = log_buffer.find(b"\r\n")
                if end == -1:
                    break

                msg = log_buffer[start:end]
                log_buffer = log_buffer[end + 1 :]

                if msg:
                    try:
                        unwrapped_message = remove_ascii_crc(msg)
                        data["LOGS_RECEIVED"] += 1
                        yield unwrapped_message

                        # Reset timeout, we just got a log!
                        expire = time.monotonic() + timeout
                    except Exception as e:
                        # Report the specific error and break.
                        data["RX_ERRS"] += 1
                        send_data(name, output, {"RX_ERR_DETAILS": str(e)})
                        return

    def gps_command(ser, command, *args, expect_reply=True):
        # Ignored fields can be empty ("")
        port = "THISPORT"
        seq = "0"
        idle = "0"
        tstatus = "UNKNOWN"
        week = "0"
        second = "0.0"
        rstatus = "0"
        reserved = "0"
        version = "0"

        cstr = (
            f"{command}A,{port},{seq},{idle},{tstatus},{week},{second},{rstatus}"
            + f",{reserved},{version};"
            + ",".join(args)
        )

        msg = add_ascii_crc(cstr) + b"\n"
        ser_write(ser, msg)

        if expect_reply:
            for log in yield_logs(ser, timeout=1.0):
                if log is None:  # Check for early return from yield_logs due to error
                    break
                if log.split(",")[0] == command + "R":
                    return
                else:
                    yield log

    systems = set()

    def process_log(log):
        nonlocal data
        nonlocal systems
        log_name = log.split(",")[0]

        key = "NUM_{}".format(log_name)
        data[key] = data.get(key, 0) + 1

        parts = log.split(";")[1].split(",")

        if log_name == "HWMONITORA":
            num_measurements = int(parts[0])

            assert num_measurements * 2 + 1 == len(parts), (
                num_measurements,
                len(parts),
            )

            for reading, status in zip(parts[1::2], parts[2::2]):
                reading = float(reading)
                status = int(status, 16)
                reading_type = status >> 8

                monitor_type = HWMONITOR_TYPES[reading_type]
                if monitor_type.startswith("TEMP"):
                    reading += DEG_C_TO_DEG_K

                data[monitor_type] = "{:.3f}".format(reading)
        elif log_name == "BESTSATSA":
            num_sats = int(parts[0])

            assert num_sats * 4 + 1 == len(parts), (num_sats, len(parts))

            for system, sat_id, status, signal_mask in zip(
                parts[1::4], parts[2::4], parts[3::4], parts[4::4]
            ):
                systems.add(system)

            for system in systems:
                data["NUM_{}_SATS".format(system)] = 0

            for system, sat_id, status, signal_mask in zip(
                parts[1::4], parts[2::4], parts[3::4], parts[4::4]
            ):
                data["NUM_{}_SATS".format(system)] += 1
        elif log_name == "BESTXYZA":
            assert len(parts) == 28, len(parts)
            data["HAVE_POS"] = int(parts[0] == "SOL_COMPUTED")
            data["HAVE_VEL"] = int(parts[8] == "SOL_COMPUTED")
            data["NUM_SATS"] = int(parts[20])
            data["NUM_SATS_IN_SOL"] = int(parts[21])
            data["P-X"] = float(parts[2])
            data["P-Y"] = float(parts[3])
            data["P-Z"] = float(parts[4])
            data["PSD-X"] = float(parts[5])
            data["PSD-Y"] = float(parts[6])
            data["PSD-Z"] = float(parts[7])
            data["V-X"] = float(parts[10])
            data["V-Y"] = float(parts[11])
            data["V-Z"] = float(parts[12])
            data["VSD-X"] = float(parts[13])
            data["VSD-Y"] = float(parts[14])
            data["VSD-Z"] = float(parts[15])
        elif log_name == "CLOCKSTEERINGA":
            data["CLK_SOURCE"] = parts[0]
            data["CLK_STATE"] = parts[1]
            data["CLK_PERIOD"] = int(parts[2])
            data["CLK_PULSEWIDTH"] = float(parts[3])
            data["CLK_BW"] = float(parts[4])
            data["CLK_SLOPE"] = float(parts[5])
            data["CLK_OFFSET"] = float(parts[6])
            data["CLK_DRIFTRATE"] = float(parts[7])
        elif log_name == "CLOCKMODELA":
            data["CLK_STATUS"] = parts[0]
            data["CLK_REJECTCOUNT"] = int(parts[1])
            data["CLK_PROPAGATIONTIME"] = float(parts[2])
            data["CLK_UPDATETIME"] = float(parts[3])
            data["CLK_BIAS"] = float(parts[4])
            data["CLK_BIASRATE"] = float(parts[5])
            data["CLK_BIAS_VARIANCE"] = float(parts[7])
            data["CLK_BIAS_COVARIANCE"] = float(parts[8])
            data["CLK_BIASRATE_VARIANCE"] = float(parts[11])
            #data["CLK_BIAS_INSTANT"] = float(parts[16])
            #data["CLK_BIASRATE_INSTANT"] = float(parts[17])
        elif log_name == "VERSIONA":
            data["GPSCARD_MDOEL"] = parts[2]
            data["GPSCARD_PSN"] = parts[3]
            data["GPSCARD_HWVERSION"] = parts[4]
            data["GPSCARD_SWVERSION"] = parts[5]
            data["GPSCARD_BOOTVERSION"] = parts[6]

    while running.is_set():
        with serial.Serial(
            port="/dev/ttyLP1", baudrate=9600, write_timeout=1, timeout=0
        ) as ser:
            ser.baudrate = 9600
            set_gps_reset(lock, reset=False)
            set_gps_power(lock, power=False)
            set_osc_power(lock, power=False)

            time.sleep(10)
            _ = ser.read(256)

            check_report()

            set_gps_reset(lock, reset=False)
            set_gps_power(lock, power=True)
            set_osc_power(lock, power=True)
            data["POWER_CYCLES"] += 1

            if not wait_for_gps_ready(ser):
                # Power cycle and try again!
                continue

            time.sleep(1)
            set_gps_reset(lock, reset=True)
            time.sleep(1)
            set_gps_reset(lock, reset=False)
            data["RESETS"] += 1

            if not wait_for_gps_ready(ser):
                # Power cycle and try again!
                continue

            log_buffer = bytearray()

            # Ask for the GPS VERSION once to verify two way comms
            list(
                gps_command(
                    ser, "LOG", "THISPORT", "VERSIONA", "ONCE", "0.0", "0.0", "NOHOLD"
                )
            )
            have_version = False
            try:
                for log in yield_logs(ser, timeout=2):
                    have_version = have_version or log.split(",")[0] == "VERSIONA"
            except TimeoutError:
                pass

            if not have_version:
                data["BOOT_TIMEOUTS"] += 1
                continue

            list(
                gps_command(
                    ser,
                    "SERIALCONFIG",
                    "THISPORT",
                    str(GPS_BAUDRATE),
                    "N",
                    "8",
                    "1",
                    "N",
                    "ON",
                )
            )

            time.sleep(0.100)

            ser.baudrate = GPS_BAUDRATE
            _ = ser.read(256)
            log_buffer = bytearray()

            # Ask for the GPS VERSION after baud rate change
            list(
                gps_command(
                    ser, "LOG", "THISPORT", "VERSIONA", "ONCE", "0.0", "0.0", "NOHOLD"
                )
            )
            have_version = False
            try:
                for log in yield_logs(ser, timeout=2):
                    have_version = have_version or log.split(",")[0] == "VERSIONA"
            except TimeoutError:
                pass

            if not have_version:
                data["BOOT_TIMEOUTS"] += 1
                continue

            listen_expire = time.monotonic() + GPS_LISTEN_TIME
            command_expire = time.monotonic() + GPS_COMMAND_INTERVAL

            # Turn on HWMONITOR, BESTSATS and BESTXYZ at a 1 Hz cadence
            for log in gps_command(
                ser, "LOG", "THISPORT", "HWMONITORA", "ONTIME", "1.0", "0.0", "NOHOLD"
            ):
                process_log(log)
            for log in gps_command(
                ser, "LOG", "THISPORT", "BESTSATSA", "ONTIME", "1.0", "0.0", "NOHOLD"
            ):
                process_log(log)
            for log in gps_command(
                ser, "LOG", "THISPORT", "BESTXYZA", "ONTIME", "1.0", "0.0", "NOHOLD"
            ):
                process_log(log)
            
            # Additionally, turn on CLOCKSTEERING, CLOCKMODEL at a 1 Hz cadence
            for log in gps_command(
                ser, "LOG", "THISPORT", "CLOCKSTEERINGA", "ONTIME", "1.0", "0.0", "NOHOLD"
            ):
                process_log(log)
            for log in gps_command(
                ser, "LOG", "THISPORT", "CLOCKMODELA", "ONTIME", "1.0", "0.0", "NOHOLD"
            ):
                process_log(log)

            # Force the use of external oscillator
            list(
                gps_command(
                    ser, "EXTERNALCLOCK", "USER", "10MHZ", "1.0e-35", "1.0e-35", "1.0e-35"
                )
            )

            list(
                gps_command(
                    ser, "CLOCKCALIBRATE", "SET", "4400", "2400", "0.08", "0.01"
                )
            )

            for log in yield_logs(ser, timeout=3):
                process_log(log)

                if not running.is_set():
                    break

                now = time.monotonic()
                if now > listen_expire:
                    break

                # Periodically send something to GPS to mask sure we can still talk to
                # it.
                if now > command_expire:
                    for log in gps_command(
                        ser,
                        "LOG",
                        "THISPORT",
                        "VERSIONA",
                        "ONCE",
                        "0.0",
                        "0.0",
                        "NOHOLD",
                    ):
                        process_log(log)

                    command_expire = time.monotonic() + GPS_COMMAND_INTERVAL

######################################################################
#                   MAIN
######################################################################

RUNNER = "RUNNER"

HIGH_PRIORITY = -1
NORMAL_PRIORITY = 0
LOW_PRIORITY = 1

# List of test drivers
TESTS = {
    "UPTIME": (uptime, NORMAL_PRIORITY),
    "BOARD_MON": (board_mon, NORMAL_PRIORITY),
    "GPS": (gps, NORMAL_PRIORITY),
}

if not HAVE_GPS:
    del TESTS["GPS"]

# Make sure no test is using runner key
assert RUNNER not in TESTS, "RUNNING not a valid test name"

PROCESS_REPORT_INTERVAL = 10  # seconds
PRINTS_PER_LOOP = 5


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("-t", "--test", type=str, default=None)
    args = parser.parse_args()
    output = Queue()
    lock = Lock()
    running = Event()
    running.set()

    if args.test is not None:
        tests = {}
        tests[args.test] = TESTS[args.test]
    else:
        tests = TESTS

    running_tests = {}
    restart_counts = {}

    # Put top manager at highest priority
    set_scheduler(HIGH_PRIORITY)

    # Initialize process objects for each test.
    for test_name, (test_function, nice) in tests.items():
        running_tests[test_name] = Process(
            target=test_function, args=(test_name, output, running, lock, nice)
        )
        restart_counts[test_name] = 0

    for test_name in running_tests:
        running_tests[test_name].start()

    last_process_report = time.monotonic()

    try:
        # Output from test will be written to linux console (e.g. A35 console).
        #with open("/dev/console", "wb") as console:
        # Hard code the destination
        UDP_IP = "172.16.3.2"
        UDP_PORT = 4242

        # Prepare outgoing socket.
        logging_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        logging_sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)

        # Output from test will be written to RadGUI
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as console:
            while True:
                for _ in range(PRINTS_PER_LOOP):
                    try:
                        # Pend for 1 second if output queue is empty (e.g. sleep).
                        #console.write(
                        #    (output.get(block=True, timeout=1) + "\n\r").encode()
                        console.sendto(
                            bytes((output.get(block=True, timeout=1) + "\n\r").encode()),
                            (UDP_IP, UDP_PORT)
                        )
                        # console.flush()
                    except queue.Empty:
                        # If queue is empty, check on maintaince tasks.
                        break

                now = time.monotonic()

                # Generate line about state of test subprocesses periodically.
                if (now - last_process_report) > PROCESS_REPORT_INTERVAL:
                    data = {}
                    for test_name, count in restart_counts.items():
                        data["{}_restarts".format(test_name)] = count
                        data["{}_alive".format(test_name)] = int(
                            running_tests[test_name].is_alive()
                        )

                    runner_string = " ".join(
                        "{}:{}".format(key, data[key]) for key in sorted(data)
                    )
                    output.put(f"{RUNNER} {runner_string}")

                    last_process_report = now

                # Check if any subtests have terminated and restart them now
                for test_name in running_tests:
                    # Check if any test stopped running and restart it
                    if not running_tests[test_name].is_alive():
                        running_tests[test_name].join()
                        restart_counts[test_name] += 1
                        test_function, nice = tests[test_name]
                        running_tests[test_name] = Process(
                            target=test_function,
                            args=(test_name, output, running, lock, nice),
                        )
                        running_tests[test_name].start()
    except KeyboardInterrupt:
        # Stop in progress tests upon control-C.
        running.clear()

        for process in running_tests.values():
            process.join()


if __name__ == "__main__":
    main()